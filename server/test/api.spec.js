...
describe("API server", () => {
  ...
  // it("responds to an unknown message id with a 404", (done) => {
  ...
  it("GET /messages/:id/comments", async () => {
    const res = await request(api).post("/messages/3/comments").send(testData);
    expect(res.statusCode).toEqual(200);
  });

  it("GET /messages/:id/reacts", async () => {
    const res = await request(api).post("/messages/3/reacts").send(testData);
    expect(res.statusCode).toEqual(200);
  });

  it("DELETE /messages/:id", async () => {
    const res = await request(api).delete("/messages/3").send(testData);
